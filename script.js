document.addEventListener('DOMContentLoaded', async () => {
    ...
    const main = async () => {
        // *** FIX: Add a check for essential elements before starting ***
    ...
    function updateAllUI() { 
        ...
        if (pointsValueTop) pointsValueTop.textContent = newPoints.toLocaleString(); 
        ...
        if (profilePointsEl) profilePointsEl.textContent = Math.floor(user.points).toLocaleString(); 
        if (pullsAmountEl) pullsAmountEl.textContent = user.pulls; 
    ...
    function updateButtonStates() { 
        if (isSpinning || !user?.id || !pullButton || !watchAdButton || !resultText) return; 
        if (user.pulls > 0) { 
    ...
    function renderDailyStreak() { 
        ...
        STREAK_REWARDS.forEach(reward => { 
            ...
            itemEl.className = 'streak-item'; 
            if (isClaimable) itemEl.classList.add('active'); 
            if (isClaimed) itemEl.classList.add('claimed'); 
            let rewardString = ''; 
            if (reward.points) rewardString += `<strong>+${reward.points.toLocaleString()}</strong> Points`; 
            if (reward.points && reward.pulls) rewardString += ' & '; 
            if (reward.pulls) rewardString += `<strong>+${reward.pulls}</strong> Pulls`; 
            let buttonText = 'Locked'; 
            let buttonClass = ''; 
            if (isClaimable) { buttonText = 'Claim'; buttonClass = 'claimable'; } 
            if (isClaimed) { buttonText = 'Claimed'; buttonClass = 'claimed'; } 
            itemEl.innerHTML = `<div class="streak-info"><h3>Day ${reward.day}</h3><p>${rewardString}</p></div><button class="streak-claim-button ${buttonClass}" ${!isClaimable ? 'disabled' : ''}>${buttonText}</button>`; 
            if (isClaimable) { 
                const button = itemEl.querySelector('button');
                if (button) button.onclick = () => claimStreakReward(reward.day); 
            } 
            streakContainer.appendChild(itemEl); 
        }); 
    }
    ...
    function updateTimedRewards() { 
        if (!treasureBtnFloat || !treasureTimerFloat || !ticketBtnFloat || !ticketTimerFloat) return;
        const now = Date.now(); 
